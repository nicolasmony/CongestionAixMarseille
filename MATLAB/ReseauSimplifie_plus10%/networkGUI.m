...
hpanelmain = uipanel(hfig, 'Position', [0.2 0 0.8 1], 'BackgroundColor', 'w');
...
hnodes = sort(hnodes);
set(hnodes, 'Marker', 'o', 'HitTest', 'on', 'MarkerFaceColor', [0.2 0.2 0.2])
...
hNodeNumbers = text(node.X,node.Y, cellstr(num2str((1:node.count)')), 'horizontalalignment', 'center');
set(hNodeNumbers, 'PickableParts', 'none', 'Visible', 'off')
hLinkNumbers = text(xLinkText+dx, yLinkText+dy, cellstr(num2str((1:link.count)')), 'horizontalalignment', 'center');
